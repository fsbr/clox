...
Compiler* current = NULL;                       // breaks concurrency.  idk why though
...
static void emitLoop(int loopStart) {
    ...
    int offset = currentChunk()->count - loopStart + 2;         // accounts for OP_LOOP's own operands
...
static int resolveLocal(Compiler* compiler, Token* name) {
    // we walk the array backwards sot hat we find the last declared variable with the identifier
    // That ensures that inner local variables correct shadow locals with the same name in surrounding
    // scopes
...
static void declareVariable() {
    ...
    // error checking for two variables declared in the same scope;
...
static void whileStatement() {
    ...
    patchJump(exitJump);    // recall patchJump is because we dont emit the correct value to jump from until later.
    emitByte(OP_POP);       // so the patchJump is only needed for forward jumps 
}
