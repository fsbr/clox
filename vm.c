...
// vm.ip +=2 is evaluated and we do nothing. and the most important hting is the (uint16_t)... part
#define READ_SHORT() \
...
//    } while (0)
    ...
    disassembleInstruction(vm.chunk, (int)(vm.ip - vm.chunk->code));        // it isnt clear to me why the program code isn't just in one huge chunk.... there's going to be multiple chunks maybe.
        ...
        switch (instruction = READ_BYTE()) {
            ...
            case OP_GET_LOCAL: {
                uint8_t slot = READ_BYTE();
                push(vm.stack[slot]);           // push on top of stack because that's where relevant data is.
                break;
            }
            case OP_GET_GLOBAL: {
                ...
                if (!tableGet(&vm.globals, name, &value)) {
                    runtimeError("Undefined Variable '%s'.", name->chars); // look how easy it is to put in the error this way, i mean i guess its not that hard in the ast way either but still its cool.
                    return INTERPRET_RUNTIME_ERROR;
                }
                push(value);
                break;
            ...
            case OP_DEFINE_GLOBAL: {
                ObjString* name = READ_STRING();
                tableSet(&vm.globals, name, peek(0));       // the variable lives on the stcak
                pop();                                      // popping after the variable is added to the table is done because GC might remove it 
                break;
            }
            case OP_SET_GLOBAL: {
                ObjString* name = READ_STRING();
